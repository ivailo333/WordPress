...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
	class Apus_Themer_Widget_Woo_Price_Filter extends WC_Widget {

		public function __construct() {
			$this->widget_cssclass    = 'apus_widget apus_widget_price_filter woocommerce widget_price_filter';
			$this->widget_description = __( 'Shows a price filter list in a widget which lets you narrow down the list of shown products when viewing product categories.', 'apus-themer' );
			$this->widget_id          = 'apus_woocommerce_price_filter';
			$this->widget_name        = __( 'WooCommerce Price Filter List', 'apus-themer' );
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => __( 'Filter by price', 'apus-themer' ),
				...
				'price_range_size' => array(
					...
					'max'   => '',
					'std'   => 50,
					'label' => __( 'Price range size', 'apus-themer' )
				),
				'max_price_ranges' => array(
					...
					'max'   => '',
					'std'   => 10,
					'label' => __( 'Max price ranges', 'apus-themer' )
				),
				'hide_empty_ranges' => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => __( 'Hide empty price ranges', 'apus-themer' )
				)
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( strlen( $min_price ) > 0 ) {
				$output .= '<li><a href="' . esc_url( $link ) . '">' . esc_html__( 'All', 'apus-framework' ) . '</a></li>';
			} else {
	            $output .= '<li class="current">' . esc_html__( 'All', 'apus-framework' ) . '</li>';
			}
	...
	register_widget( 'Apus_Themer_Widget_Woo_Price_Filter' );
}